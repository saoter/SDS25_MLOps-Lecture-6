...
X_train, X_test, y_train, y_test, train_ids, test_ids = train_test_split(
    X, y_encoded, animal_ids, test_size=0.3, stratify=y_encoded, random_state=42
)
...
# Attach label encoder
model_pipeline.label_encoder = label_encoder

# Save model
Path("models").mkdir(exist_ok=True)
joblib.dump(model_pipeline, "models/model_2.pkl")

# --- Evaluate function
def evaluate(X, y_true, ids, label):
...
# --- Evaluate both sets
acc, prec, rec, f1, test_predictions = evaluate(X_test, y_test, test_ids, "Test")
_, _, _, _, train_predictions = evaluate(X_train, y_train, train_ids, "Train")
...
print(f"🆕 Trained model saved with model_id = {model_id}")
...
# --- Insert evaluation into MODEL_EVALUATIONS
cursor.execute('''
...
conn.commit()
conn.close()
print("✅ New model trained, evaluated, and all results logged.")
